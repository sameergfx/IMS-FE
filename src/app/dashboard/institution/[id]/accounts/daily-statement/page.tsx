...
export default function DailyStatementPage() {
  ...
  const totals = (groups: DailyStatementGroup[], clickable = false) => <div className={styles.tableWrap}>
    <table className={styles.table}><thead><tr><th>{clickable ? "Category" : "Payment method"}</th><th>Received</th><th>Expenses</th></tr></thead>
  ...
  return <div className={`${styles.page} ${printStyles.report}`}>
    ...
    {!day ? <p className={styles.state}>Select a date.</p> : loading ? <p className={styles.state} role="status">Loading daily statement...</p> : error ? <p className={styles.state} role="alert">{error}</p> : current && <>
      <div className={styles.reportHeading}><h2 className={styles.institution}>{current.institution_name}</h2><div className={styles.reportDates}><p className={styles.date}>Statement Date: {current.date}</p>{generatedOn && <p className={styles.date}>Generated On: <time dateTime={generatedOn.toISOString()}>{generatedOn.toLocaleString("en-IN", { timeZone: "Asia/Kolkata", day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: true })} IST</time></p>}</div></div>
      <div className={styles.summary}><div><span>Opening balance</span><strong>{money(current.opening_balance)}</strong></div><div><span>Total received</span><strong>{money(current.total_received)}</strong></div><div><span>Total expenses</span><strong>{money(current.total_expenses)}</strong></div><div><span>Net movement</span><strong>{money(current.net_movement)}</strong></div><div><span>Closing balance</span><strong>{money(current.closing_balance)}</strong></div></div>
      {!current.entries.length && <p className={styles.state}>No transactions on this date.</p>}
      <div className={styles.breakdowns}><section className={styles.section}><h2 className={styles.sectionTitle}>Category breakdown</h2>{totals(current.categories, true)}</section>
